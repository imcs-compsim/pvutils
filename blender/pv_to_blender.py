...
def surface_to_blender(item, name, base_dir, time_steps=None,
        frames=None, frame_interval=1, set_time_step_function=None):
    """
    ...
    pa.SaveData(os.path.join(base_dir, name + '_mesh.ply'), proxy=item,
        FileType='Binary')

    # Get group data.
    vtk_data = pvutils.get_vtk_data_as_numpy(item, cell_data=True,
    ...
    if 'blender_cell_group' in cell_data.keys():
        ...
        for i_cell, cell_group in enumerate(cell_data['blender_cell_group']):
            ...
            blender_cell_group[cell_group_int].extend(
                cell_connectivity[i_cell])

        # Extract the unique vertices.
        for group_id in blender_cell_group.keys():
            blender_cell_group[group_id] = list(
                set(blender_cell_group[group_id]))
    ...
    for i_time, time in enumerate(time_steps):
        ...
        if i_time == 0:
            position = np.zeros(
                [len(time_steps), len(vtk_data['coordinates']), 3])
    ...
    # Save data.
    np.save(os.path.join(base_dir, name + '_data'),
        {
            'position': position,
            'frames': frames,
...
def fibers_to_blender(item, file_name, time_steps=None, n_segments=5,
        frames=None, frame_interval=1, set_time_step_function=None):
    """
    ...
    # Get the vtk data.
    vtk_data = pvutils.get_vtk_data_as_numpy(item, coordinates=True,
        point_data=True, cell_connectivity=True)
    ...
    for i_fiber, local_connectivity in enumerate(vtk_connectivity):
        ...
        cell_topology = [last_index + i
            for i in range(n_elements_per_fiber[i_fiber] + 1)]
    ...
    for i_time, time in enumerate(time_steps):
        ...
        for i_cell in range(n_elements_total):
            ...
            cell_positions = vtk_data['coordinates'][
                cell_relevant_nodes[i_cell]]
