...
def set_keyframe_sequence(shape_key, frames, i_frame):
    ...
    if i_frame == 0:
        ...
        if i_frame == len(frames) - 1:
            ...
            values = [0.0, 1.0, 0.0]
            values_frames = [frames[i_frame + i_inner]
                for i_inner in [-1, 0, 1]]
...
def create_fiber(nodes, pv_data, i_fiber):
    ...
    for i_frame in range(len(frames)):
        ...
        set_fiber_position(shape_key, nodes, pv_data[b'coordinates'][i_frame],
...
def load_surface(name, base_dir):
    ...
    bpy.ops.import_mesh.ply(
        filepath=os.path.join(base_dir, name + '_mesh.ply'))
    obj = bpy.context.active_object
    obj.name = name
    pv_data = np.load(os.path.join(base_dir, name + '_data.npy'),
    ...
    for i_frame in range(len(frames)):
        ...
        print('Finished surface "{}" for frame {}/{}'.format(
