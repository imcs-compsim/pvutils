...
from meshpy import (InputFile, mpy, MaterialReissner, Beam3rHerm2Line3,
...
cubit = CubitPy()
solid_cantilever = create_brick(cubit, 1, 1, 30, mesh_interval=[2, 2, 5],
    element_type=cupy.element_type.hex27)
cubit.add_node_set(solid_cantilever.surfaces()[1], name='solid_fix',
        bc_type=cupy.bc_type.dirichlet,
        bc_description='NUMDOF 3 ONOFF 1 1 1 VAL 0 0 0 FUNCT 0 0 0')
cubit.add_node_set(solid_cantilever.surfaces()[0], name='solid_load',
        bc_type=cupy.bc_type.neumann,
        bc_description='NUMDOF 3 ONOFF 1 1 0 VAL {0} {0} 0 FUNCT 1 1 0'.format(
            load))
...
# Add beam.
beam_set = create_beam_mesh_line(mesh, Beam3rHerm2Line3, mat,
    [4, 0, -15], [4, 0, 15], n_el=5)
mesh.add(BoundaryCondition(beam_set['start'],
        ('NUMDOF 9 ONOFF 1 1 1 1 1 1 0 0 0 VAL 0 0 0 0 0 0 0 0 0 '
        + 'FUNCT 0 0 0 0 0 0 0 0 0'),
        bc_type=mpy.bc.dirichlet
    ))
mesh.add(BoundaryCondition(beam_set['end'],
...
# Set headers.
set_header_static(mesh, time_step=1.0, n_steps=1, tol_residuum=1e-10,
    tol_increment=1e-10)
